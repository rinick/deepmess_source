---
layout: page
title:  "关于"
---

作者：Rinick [[微博](http://www.weibo.com/rinick)] [[Github](https://github.com/rinick)]

开发环境：<a href="https://www.dartlang.org" target="_blank">Google Dart</a>

免费建站服务：<a href="https://pages.github.com" target="_blank">Github Pages</a>

表情作者：可而 [<a href="http://www.weibo.com/u/1714554521" target="_blank">微博</a>]

网站主题：<a href="https://github.com/jasonlong/architect-theme" target="_blank">Architect Theme</a>

老版本：[MD5大作战](http://www.goozo.net/index.php?title=MD5%E5%A4%A7%E4%BD%9C%E6%88%98) (需要Flashplayer)


### 内容版权

未经作者许可，不得将**名字竞技场**生成的内容用于任何商业目的，内容包括：

* 系统预设的图标和表情
* 系统根据名字生成的徽章图标
* 系统生成的战斗过程

非商用的情况下，允许使用以上内容，但须注明出处。

### 第三方API授权

[用iframe方式调用名字竞技场API](./api.html) 可免费用于商业目的。但使用过程中

* 不得对**名字竞技场**本身的文件或算法做出改变
* 不得对**名字竞技场**中生成内容做出改变（[自定义语言包](./custom_lan.html)除外）
* 不得故意遮挡或改变**名字竞技场**中的游戏标题或广告
* 不得将生成的内在容脱离**名字竞技场**的场合下使用